...
The confusion matrix shows we correctly predicted No `r tn` times and Yes `r tp` times for an overall accuracy of 77.54%. Our model has higher recall and precision. This means that our model is better at predicting if someone will stay at their current position as oppose to looking for a new one.
...
Now with the best parameters we train the full model on the trainined set to obtain our finalized model.
...
Our model has an roc_auc of `r roc_auc` on the test set which is a about the same on the training set. This implies the model has generalized on the training set after the down sampling.
...
The most important feature is city_development_index by a wide margin. Next is the city and relevant experience followed by university and base experience. The city_development_index would be a variable to take a closer look at in the future.
...
The confusion matrix shows we correctly predicted an individual will stay at their current job `r tp` times and is looking for a new position `r tn` times. Our model has higher recall (Neg Pred Valu) than precision (Pos Pred Value) which states it can catch false negatives better than false positives. A false negative is when the model predicts an individual will leave but they are actually going to stay, a false positive is when the model predicts an individual will stay but they are planning to leave.
