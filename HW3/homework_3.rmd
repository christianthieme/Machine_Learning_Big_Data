...
Looking at the above summary table, we can see that (NEED TO ADD SOME DESCRIPTION HERE). Futher, we'll look at each of these features distribution: 
...
train.data <- preproc.param %>% predict(train.data)
test.data <- preproc.param %>% predict(test.data)
...
Prediction accuracy of LDA is comparing prediction result from the model output with the actual data.
