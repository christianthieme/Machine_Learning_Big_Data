...
library(FNN)
# K = 15 
classifier_knn <- kknn( knn_train[,1] ~ .,
                          knn_train[,-1],
                          knn_test[,-1],
                          k = 15,
                          #distance = 10,
...
cM <- table( knn_test[,1], fitted(classifier_knn))
accuracy <- (cM[1]+cM[4])/(cM[1]+cM[2]+cM[3]+cM[4])

print(paste('Accuracy (k=15):', accuracy))
...
Listing the Accuracy of all our models: 
...
Based on the prediction accuracy of the LDA , the KNN, the Decision tree and the Random Forest models, we can  say that the LDA had the better accuracy. The LDA is the better performing model of the four. 
